...
const Table = React.memo(
  ({ titles, page, episode, pagination, setPage, NextPage }) => {
    let filledArray = useMemo(
      () =>
        [...new Array(episode?.info?.pages)]?.map((item, i) => (
          <button
            key={i}
            type="button"
            className={cx(
          ...
          <table id="table" className="min-w-full leading-normal">
            <thead>
              <tr>
                ...
                {!Object.keys(titles)?.includes("dimension") && (
                  <th
                    scope="col"
                    className="px-5 py-3 bg-white  border-b border-gray-200 text-gray-800  text-left text-sm uppercase font-semi"
                  ></th>
