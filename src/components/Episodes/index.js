...
const NextPage = ({ page }) => {
  const { episode, isLoading } = useEpisode({ page: page + 1 });
  return (
    <span className="display-none" users={episode} isLoading={isLoading}></span>
  );
};
...
const Episodes = () => {
  ...
  let filledArray = [...new Array(episode?.info?.pages)]?.map((item, i) => (
    <button
      key={i}
      type="button"
      className={cx(
      ...
      <div className="py-8">
        <div className="flex justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-gray-400 dark:text-gray-200">
              Episode Count : {episode?.info?.count}
            </p>
          </div>
        ...
        <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
          <div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
            <table id="table" className="min-w-full leading-normal">
              <thead>
                <tr>
                  <th
                    scope="col"
                    className="px-5 py-3 bg-white  border-b border-gray-200 text-gray-800  text-left text-sm uppercase font-semi"
                  >
                    Episode Name
                  </th>
                  <th
                  ...
                  <th
                    scope="col"
                    className="px-5 py-3 bg-white  border-b border-gray-200 text-gray-800  text-center text-sm uppercase font-semi"
                  >
                    Character Count in Episode
                  </th>
                  <th
                    scope="col"
                    className="px-5 py-3 bg-white  border-b border-gray-200 text-gray-800  text-left text-sm uppercase font-semi"
            ...
            <div className="px-5 bg-white py-5 flex flex-col xs:flex-row items-center xs:justify-between">
              <div className="flex items-center">{filledArray}</div>
              {page + 1 <= episode?.info?.pages && <NextPage page={page} />}
            </div>
          </div>
        </div>
      </div>
